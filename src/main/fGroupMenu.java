...
public class fGroupMenu extends JFrame {
    ...
    public void ShowTableData() 
    {
    	...
    	NhomThucDonDAO dao = new NhomThucDonDAO();
        ...
        List<Nhomthucdon> list = dao.Load();
        for (int i = 0; i < list.size(); i++) {
        	Nhomthucdon entity = (Nhomthucdon)list.get(i);
        	Vector row = new Vector();
            row.add(entity.getMaNhom());
            row.add(entity.getTenNhom());
    ...
    private String EditData(String key, String ten, String dienGiai, boolean status)
    {
    	String errMessage = "";
    	try {
    		NhomThucDonDAO dao = new NhomThucDonDAO();
			...
			{
				dao.UpdateData(key, ten, dienGiai, status);
    ...
    private String InsertData()
    {
		...
		if(errMessage.length()<1)
		{
			Nhomthucdon entity = new Nhomthucdon();
			Date currentDate = new Date();
			try
			{
				entity.setMaNhom(txtMa.getText());
				entity.setTenNhom(txtTen.getText());
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					fGroupMenu frame = new fGroupMenu();
	...
	public fGroupMenu() {
		...
		addWindowListener(new WindowAdapter() {
			...
			public void windowClosing(WindowEvent arg0) {
				System.out.println("Thoát Danh mục -> Nhóm thực đơn");
			}							
				...
				try {				
					if (e.getSource() == tbl) {
			            ...
			            if (iDongDaChon != -1) {
			                ...
			                String key = tbl.getValueAt(iDongDaChon, 0).toString();
			                String ten = tbl.getValueAt(iDongDaChon, 1).toString();
